...
#include <prometheus/command_line_parser/regex.hpp>
...
namespace prometheus::clp
{
	...
	public:
		...
		using parser_type = OptionParser<string_type>;
		friend parser_type;
	...
	public:
		...
		[[nodiscard]] constexpr auto as() const -> std::optional<T>
		{
			...
			else
			{
				...
				if (current_value_ == secret_value)
				{
					if (option_short_format_.empty())
					{
						std::println(
							stderr,
							"Error: Option `{}` uses an incorrect format.(use --{}=xxx)",
							option_long_format_,
							option_long_format_
						);
					}
					else
					{
						std::println(
							stderr,
							"Error: Option `{}` uses an incorrect format.(use --{}=xxx or --{}=xxx)",
							option_long_format_,
							option_long_format_,
							option_short_format_
						);
					}
				}
				else
				{
					std::println(
						stderr,
						"Error: Option `{}` has an invalid value.(cannot parse `{}` as `{}`)",
						option_long_format_,
						current_value_,
						meta::name_of<T>()
					);
