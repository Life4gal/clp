...
#ifndef PROMETHEUS_CLP_USE_EXPECTED
#define PROMETHEUS_CLP_USE_EXPECTED 0
#endif

#if PROMETHEUS_CLP_USE_EXPECTED
#include <expected>
...
namespace prometheus::clp::regex
{
#ifndef PROMETHEUS_CLP_IDENTIFIER
#define PROMETHEUS_CLP_IDENTIFIER "[[:alnum:]][-_[:alnum:]\\.]*"
#endif

#ifndef PROMETHEUS_CLP_LIST_SEPARATOR
#define PROMETHEUS_CLP_LIST_SEPARATOR ","
#endif

#define PROMETHEUS_CLP_LIST_SEPARATOR_IGNORE_WS PROMETHEUS_CLP_LIST_SEPARATOR "\\s*"
	...
	template<string_type Range>
	using regex_sub_match_type = regex_match_result_type<Range>::value_type;
	template<string_type Range>
	...
	constexpr std::basic_string_view<char_type> pattern_list
	{
			...
			"(?:" PROMETHEUS_CLP_LIST_SEPARATOR_IGNORE_WS PROMETHEUS_CLP_IDENTIFIER ")*"
	};
	...
	{
			PROMETHEUS_CLP_LIST_SEPARATOR_IGNORE_WS
	};
	...
	struct descriptor_integer
	{
		...
		std::basic_string_view<char_type> value; // NOLINT(clang-diagnostic-padded)
	};
	...
	template<typename Range>
	[[nodiscard]] constexpr auto sub_match_to_string_view(const regex_sub_match_type<Range>& sub_match) noexcept -> std::basic_string_view<char_type>
	{
	...
	struct parser
	{
...
#endif
		{
			if (regex::parse_boolean<true>(range))
			{
				return true;
			}

			if (regex::parse_boolean<false>(range))
			{
				return false;
			}
			...
			}
		{
			...
			{
				OutRange tmp_out{};
				if constexpr (requires { tmp_out.reserve(std::ranges::distance(view)); })
				{
					tmp_out.reserve(std::ranges::distance(view));
				}
				...
				for (auto it = std::ranges::begin(view); it != std::ranges::end(view); std::ranges::advance(it, 1))
				{
					...
					{
						tmp_out.emplace_back(*std::move(value));
					}
					else if constexpr (requires { tmp_out.push_back(*std::move(value)); }) //
					{
						tmp_out.push_back(*std::move(value));
					}
					else if constexpr (requires { tmp_out.emplace(*std::move(value)); }) //
					{
						tmp_out.emplace(*std::move(value));
					}
					else
					{
						PROMETHEUS_SEMANTIC_STATIC_UNREACHABLE();
				...
				if constexpr (requires { out.reserve(tmp_out.size()); })
				{
					out.reserve(tmp_out.size() + out.size());
				}
